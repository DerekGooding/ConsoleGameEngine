...
{
    static Sprite? _spriteSheet, _spriteSheetSMALL;
    public static FontPadding Fontpadding = new(0, 0, 0, 0);
    static int _width, _height, _widthSMALL, _heightSMALL;
    static Dictionary<char, Coords> _dictionary = new()
    { { ' ', new Coords {X= 0, Y=0 } },
    { '!', new Coords {X= 1, Y=0 } },
    { '"', new Coords {X= 2, Y=0 } },
    { '#', new Coords {X= 3, Y=0 } },
    ...
    { '}', new Coords {X= 3, Y=5 } },
    { '~', new Coords {X= 4, Y=5 } }};
    static readonly Dictionary<char, Coords> _dictionarySMALL = new()
    ...
    {
        _spriteSheet = new Sprite(fileName);
        _width = w;
        _height = h;
    }
    public static void LoadSmallFont(string fileName, int w, int h)
    {
        _spriteSheetSMALL = new Sprite(fileName);
        _widthSMALL = w;
        _heightSMALL = h;
    }
    ...
    public static void SetDictionary(Dictionary<char, Coords> _dictionary) => TextWriter._dictionary = _dictionary;
    ...
    public static Sprite GenerateTextSprite(string text, Textalignment textalignment, int fontSize, short backgroundColor = (short)GameConsole.COLOR.FG_WHITE, short foregroundColor = (short)GameConsole.COLOR.FG_BLACK, FontType fontType = FontType.standard)
    {
        ...
        {
            _width = TextWriter._width;
            _height = TextWriter._height;
            _dictonary = _dictionary;
        }
        else if (fontType == FontType.small)
        {
            _width = _widthSMALL;
            _height = _heightSMALL;
            _dictonary = _dictionarySMALL;
        }

        sprite = new Sprite(TextWriter._width * fontSize * longesLineLength, TextWriter._height * numberOfLines * fontSize);
        ...
        foreach (var str in text.Split('\n'))
        {
            ...
            {
                allignmentOffset = (longesLineLength - str.Length) * TextWriter._width * fontSize / 2;
            }
            else if (textalignment == Textalignment.Right)
            {
                allignmentOffset = (longesLineLength - str.Length) * TextWriter._width * fontSize;
            }
            ...
            for (var i = 0; i < str.Length; ++i)
            {
                ...
                if (fontType == FontType.small)
                    letter = _spriteSheetSMALL.ReturnPartialSprite((coords.X * _width) + Fontpadding.Left + coords.X + 1, (coords.Y * _height) + Fontpadding.Top + coords.Y + 1, _width, _height);
                else if (fontType == FontType.standard)
                    letter = _spriteSheet.ReturnPartialSprite((coords.X * _width) + Fontpadding.Left + coords.X + 1, (coords.Y * _height) + Fontpadding.Top + coords.Y + 1, _width, _height);
